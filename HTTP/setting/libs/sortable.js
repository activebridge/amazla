...
const setup = () => {
  ...
  // Rows render in array order, and findCardsByHandle returns them in DOM
  ...
  // removeItem before setItem: the storage bridge does not reliably fire a
  // change event for a rewrite of the same key, and without it the list would
  // keep rendering the old order.
  const saveOrder = () => {
    if (dragIndex === currentIndex || dragIndex < 0 || currentIndex < 0) return

    const items = JSON.parse(storage.getItem('actions') || '[]')
    const [moved] = items.splice(dragIndex, 1)
    items.splice(currentIndex, 0, moved)

    storage.removeItem('actions')
    storage.setItem('actions', JSON.stringify(items))
  }

  // The list itself never scrolls — only the container around it does — so it
  // is left un-clipped: hiding its overflow would cut the dragged row off at
  // the last row instead of letting it travel the whole visible area.
  const disableScroll = () => {
  ...
  const onStart = (e, clientY) => {
    // A filtered list is not in array order, so a drop would move another row.
    ...
    // The picked-up row grows and casts a shadow past its own box. Both fit
    // because LIST carries the list's padding — overflow clips at the padding
    // edge, so that padding is the bleed room this needs.
    card.style.transform = 'scale(1.02)'
    card.style.boxShadow = '0 6px 16px rgba(0, 0, 0, 0.3), 0 0 0 1px rgba(255, 255, 255, 0.5)'
