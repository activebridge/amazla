#!/usr/bin/env python3
"""Generate step icons: walking figure colored by progress (5 levels)."""
...
SRC = Image.open("/tmp/steps_icon.png").convert("RGBA")
...
for res_name, res_width in RESOLUTIONS.items():
    ...
    out_dir = os.path.join(BASE_DIR, "assets", res_name, "steps")
    os.makedirs(out_dir, exist_ok=True)

    for i, color in enumerate(LEVELS):
        icon = gen_step(sz, outline, color)
        icon.save(os.path.join(out_dir, str(i) + ".png"))

    gray = gen_step(sz, outline, (80, 80, 80))
    gray.save(os.path.join(out_dir, "gray.png"))

    print(res_name + ": 5+1 step icons " + str(sz) + "x" + str(sz))
