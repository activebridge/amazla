...
export const refreshCodes = () => {
  codeWidgets.forEach(({ dark1, main1, dark2, main2, acc, isBottom }) => {
    ...
    dark1.set({ text: half1 })
    main1.set({ text: half1 })
    dark2.set({ text: half2 })
    main2.set({ text: half2 })
  })
}
...
export const Layout = (accounts) => {
  ...
  const timerGeometry = { w: imgSize + 2, h: imgSize + 2, line_width: 5, color: ARC_BG_COLOR }
  const timerArc = arc({ ...timerGeometry, start_angle: -90, end_angle: 270 })

  return {
    updateTimer: (remaining) => {
      const rotation = -90 + (30 - remaining) * 12
      const coverage = (1 - remaining / 30) * 360
      // Repeat the geometry: `.set` re-runs the centering pipeline, so a partial
      // update would resize the arc to the screen box.
      timerArc.set({ ...timerGeometry, start_angle: rotation, end_angle: rotation + coverage })
    }
  }
}
