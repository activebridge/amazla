...
function sha512(message) {
  ...
  for (let i = 0; i < message.length; i += 128) {
    ...
    for (let j = 16; j < 80; j++) {
      const s0 = xor(xor(rotr(w[j - 15], 1), rotr(w[j - 15], 8)), shr(w[j - 15], 7))
      const s1 = xor(xor(rotr(w[j - 2], 19), rotr(w[j - 2], 61)), shr(w[j - 2], 6))
    ...
    for (let j = 0; j < 80; j++) {
      ...
      const t1 = add(hh, S1, ch, K512[j], w[j])
 ...
 * @param {string} algorithm - SHA1 (default), SHA256, or SHA512
