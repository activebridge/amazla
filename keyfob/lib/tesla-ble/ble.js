...
class TeslaBLE {
  ...
  connect(mac, callback, attemptNumber = 0) {
    ...
    this._ensureBLE().connect(mac, (result) => {
      // After first settlement, connect callback fires only on disconnect events
      if (done) {
        if (!result.connected) {
          this.connected = false
      ...
      setTimeout(() => {
        ...
        this._ensureBLE().startListener(this.profile, (response) => {
          ...
          // Safety fallback if descWriteComplete never fires
          setTimeout(() => {
            if (!done) {
  ...
  // Mirrors Tesla Go SDK: chunks are paced 20ms apart so the BLE radio has time
  // to transmit each packet before the next arrives in the OS queue.
  ...
  _handleResponse(data, _len) {
    ...
    if (this._rxBuf === null) {
      ...
      // Only check on first chunk — mid-reassembly chunks can't be duplicates.
      const now = Date.now()
    ...
    // Capture and clear before calling so the callback can register a new responseCallback
    const cb = this.responseCallback
    this.responseCallback = null
