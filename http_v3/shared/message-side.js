import { bin2hex, buf2json, buf2str, json2buf, str2buf } from './data'
...
class Session extends EventBus {
  ...
  addChunk(payload) {
    ...
    if (payload.payloadLength !== payload.payload.byteLength) {
      ...
      this.emit(
        'error',
        Error(`receive chunk data length error, expect ${payload.payloadLength} but ${payload.payload.byteLength}`),
      )
      return
    }
